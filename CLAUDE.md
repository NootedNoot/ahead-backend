...
`redProjected` (used only for RED gating, never `projected`/`projectedExtended`, which stay flat and drive YELLOW + notification text) decays off the flat-rate assumption in two ways, and they are **not symmetric on purpose**: `assessRateTrajectory` returning `'decelerating'` decays either direction (a rate genuinely easing off is genuinely easing off). But *unconfirmed* decay — `PROJECTION_DECAY_RATE_THRESHOLD`/`DEFAULT_DECAY_PER_STEP`, added 2026-08-01 — only ever softens a fast, still-accelerating **rise**; a fast, still-accelerating **fall** always keeps the flat, worst-case projection, full stop. Reasoning: a rising glucose has a natural brake (insulin catching up) that a falling one doesn't, so understating a RED low is the more dangerous failure mode of the two. If you touch this again, keep `'fast accelerating FALL ... leaves redProjected exactly flat'` in `test/trend-detector.test.js` passing — that test is the guard against this asymmetry drifting back to symmetric.
